...
const Dropzone = () => {
    ...
    const onDropRejected = () => {
        mostrarAlerta('No se pudo subir, el límite es 1Mb. Obtén una cuenta para subir archivos mas grandes');
    };
    ...
    const archivos = acceptedFiles.map(archivo => (
        <li key={archivo.lastModified} className="bg-white flex-1 p-3 mb-4 shadow-lg rounded">
            <p className="font-bold text-md text-center">{archivo.path}</p>
                        ...
                        <input className="h-100" {...getInputProps()} />
                        {
                            isDragActive ? <p className="text-2xl text-center text-gray-600">Suelta el archivo</p> :
                                <div className="text-center">
                                    <p className="text-md text-center text-gray-600">Selecciona un archivo y arrastralo aquí</p>
                                    <button
